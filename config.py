...
keys = [
    ...
    Key([mod], "space", lazy.spawn('rofi -show drun -show-icons'), desc="Run rofi"),
    Key([mod, "shift"], "s", lazy.spawn("shutter -s -o /home/ricardo/Downloads/Screenshot_%y-%m-%d_%T.png -e -C"), desc="Take screenshot"),
...
groups = [
    Group('1', label="1", 
          #matches=[Match(wm_class='firefox'), Match(wm_class='thunderbird'), Match(wm_class='qutebrowser')], 
          layout="stack"),
    Group('2', label="2", layout="monadtall"),
    Group('3', label="3", layout="columns"),
    Group('4', label="4", 
          #matches=[Match(wm_class='discord'), Match(wm_class='zoom'), Match(wm_class="teams"), Match(wm_class="whatsdesk")], 
          layout="stack"),
    Group('5', label="5", 
...
groups.append(ScratchPad('scratchpad', [
    ...
    DropDown('blueman', 'blueman-manager', width=0.05, height=0.6, x=0.35, y=0.1, opacity=1),
]))
